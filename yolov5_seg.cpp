...
bool Yolov5Seg::Detect(Mat& srcImg, Net& net, vector<OutputSeg>& output) {
	...
	// cv::imshow("net_output_img", net_output_img[1]);
    // cv::waitKey(0);
	...
	for (int i = 0; i < nms_result.size(); ++i) {
		...
		// cout<<"boxMask: "<<result.boxMask.size()<<endl;
		// cv::imshow("result.boxMask", result.boxMask);
    	// cv::waitKey(0);
	...
	// cout<<"temp_mask_proposals.size(): "<<Mat(temp_mask_proposals[0])<<endl;
	// 画分割 mask
	for (int i = 0; i < temp_mask_proposals.size(); ++i) {
		GetMask2(Mat(temp_mask_proposals[i]).t(), net_output_img[1], output[i], mask_params);
	}
	// cout<<"------------------   boxMask -------------------------    "<<endl;
	// cout<<"boxMask: "<<boxmask.size()<<endl;
